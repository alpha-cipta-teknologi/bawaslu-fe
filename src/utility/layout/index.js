...
export const resolveVerticalNavMenuItemComponent = item => {
  if (item.children) return 'VerticalMenuGroup'
  return 'VerticalMenuLink'
}
...
export const isNavLinkActive = (link, currentURL, routerProps) => {
  const childrenRoute = ['save', 'download', 'upload'].find(str => currentURL.includes(str))

  const currentChildrenURL = (currentURL || '').split(`/${ childrenRoute }`)
    ? currentURL.split(`/${ childrenRoute }`)[0]
    : currentURL

  return (childrenRoute ? currentChildrenURL === link : currentURL === link) ||
